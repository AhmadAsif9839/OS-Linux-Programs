// Returning value from threads
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
#include <time.h>
#include <pthread.h>
#include <math.h>

void *f1(void *);

int main(int argc, char *argv[]) {

    if (argc != 3) {
        printf("Please enter two arguments\n");
        exit(1);
    }

    pthread_t tid1, tid2;

    // Storing command line arguments
    int num1 = atoi(argv[1]);
    int num2 = atoi(argv[2]);

    // Creating threads
    pthread_create(&tid1, NULL, f1, (void *)&num1);
    pthread_create(&tid2, NULL, f1, (void *)&num2);

    void *rv1, *rv2;
    
    // Taking returning value by the threads
    pthread_join(tid1, &rv1);
    pthread_join(tid2, &rv2);

    // typecasting and dereferencing the pointer
    long prime_number_1 = *(long *)rv1;
    long prime_number_2 = *(long *)rv2;

    printf("The %dth prime number returned by the child thread is %ld\n\n", num1, prime_number_1);
    printf("The %dth prime number returned by the child thread is %ld\n\n", num2, prime_number_2);


    return 0;
}

void *f1(void *args) {
    int n = *(int *)args;
    long *candidate = (long *)malloc(sizeof(long));
    *candidate = 2;
    while (1) {
        int is_prime = 1;
        long factor;
        for (factor = 2; factor <= sqrt(*candidate); ++factor) {

            if ((*candidate) % 2 == 0) {
                is_prime = 0;
                break;
            }

            if ((*candidate) % factor == 0) {
                is_prime = 0;
                break;
            }
        }
        if (is_prime == 1)
            n--;
        if (n == 0) 
            // Returning pointer
            pthread_exit((void *)candidate);

        ++(*candidate);
    }
}
